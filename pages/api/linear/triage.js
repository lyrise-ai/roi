...
// All alpha user feedback is routed here before being approved into the backlog.
//
// Required env vars:
...
//                             (find it via Linear API: query { workflowStates { nodes { id name type } } })
...
export default async function handler(req, res) {
  ...
  if (!apiKey || !teamId) {
    // TODO: fill LINEAR_API_KEY and LINEAR_TEAM_ID in .env.local
    console.warn(
...
function buildDescription(description, source) {
  ...
  lines.push(
    '',
    '---',
    '*Routed automatically from alpha user feedback. Approve to move into backlog.*',
  )
