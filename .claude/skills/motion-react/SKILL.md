...
> **LyRise project binding:** before using any hardcoded duration/easing from this skill, check
> [`_lyrise-motion-tokens.md`](../_lyrise-motion-tokens.md) — this project's `tokens/motion.css`
> already defines the durations and easing curves to use instead. Brand colors, glass effect, and
> corner radii come from `tokens/colors.css` / `tokens/effects.css` / `tokens/radius.css`, never
> from this skill's own palette suggestions.
...
```tsx
<AnimatePresence>
  {isOpen && <motion.dialog exit={{ opacity: 0 }}>Content</motion.dialog>}
</AnimatePresence>
```
...
```tsx
<motion.div animate={{ height: isOpen ? 'auto' : 0 }}>Content</motion.div>
```
